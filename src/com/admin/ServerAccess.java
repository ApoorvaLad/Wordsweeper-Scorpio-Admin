 ...
 * {@link #connect(IMessageHandler)} method. Once connected, you can send a
 * message request to the server through the {@link #sendRequest(Message)}
 * message. All responses to these requests from the server are processed by a
 * {@link IMessageHandler} object.
 * <p>
 * Should you want more control, then when sending a request, you can also
 * register a {@link IController} object to specifically be given the response
 * from the server to a given request.
 * <p>
 * Once done, call {@link #disconnect()} and the server communication will be
 * terminated and the background {@link ServerThread} thread will cleanly shut
 * down.
 */
public class ServerAccess {
	...
	/**
	 * Start up reader and talker threads. Returns false if unable to contact
	 * server. When server messages are received, they are handled by the given
	 * handler object.
	 */
	public boolean connect(final IMessageHandler handler) {
		...
		// start connection by starting reader thread and send proper LOGIN
		// request.
		isActive = true;
	...
	public void disconnect() {
		...
		try {
			// Terminate connection to server and (by implication) stop
			// FromServer thread
			server.close();
			toServer = null;
			server = null;
	...
	/**
	 * Thread to receive Responses back from Server. This inner class has access
	 * to attributes defined in the enclosing ServerAccess class.
	 */
